...
| t1 | ``` UPDATE EMP SET SAL = 4000 WHERE ENAME ='Hichem'; ``` |------|Mise à jour du salaire de Hichem au niveau de la session 1|
| t2 | ------ |```UPDATE EMP SET SAL = SAL + 1000 WHERE ENAME ='Mohamed';```|Mise à jour du salaire de Mohamed au niveau de la session 1|
| t3 | ```UPDATE EMP SET SAL = SAL + 1000 WHERE ENAME ='Mohamed';```|La requête ne s'exécute pas au niveau de la session 1|
...
| t6  |```UPDATE EMP SET SAL = SAL + 1000 WHERE ENAME ='Mohamed';```| ------|La requête ne s'exécute pas au niveau de la session 1|
| t7 |  ------ |```Commit;```|  Session 1: --> 1 row updated.|
| t8 | ------ |```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem', 'Maaoui');```|Résultat final :Mohamed--> 3000 et Hichem--> 5000|
...
| t0| ``` SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem');``` |||
| t1 | ``` UPDATE EMP SET SAL = 4000 WHERE ENAME ='Hichem'; ``` |------|Mise à jour du salaire de Hichem au niveau de la session 1|
| t2 | ------ |```SET TRANSACTION ISOLATION LEVEL READ COMMITTED;```|Lorsqu'une transaction s'exécute à ce niveau d'isolement(READ COMMITTED), une requête SELECT ne voit que les données validées avant le début de la requête et ne voit jamais ni les données non validées ni les modifications validées lors de l'exécution de la requête par des transactions simultanées.|
| t3 | ------ |```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem');```|Les salaires ont les premières valeurs(la valeur atribuée au niveau de la session 1 n'est pas prise en considération). |
| t4 | ------ |```UPDATE EMP SET SAL = 3800 WHERE ENAME ='Mohamed';```|Mise à jour du salaire de Mohamed au niveau de la session 2|
| t5 | ```Insert into EMP (EMPNO,ENAME,JOB,MGR,HIREDATE,COMM,DEPTNO) values ('9999','Maaoui','Magician',null,to_date('17/02/2021','DD/MM/RR'),null,'10');``` |------|1 row created dans la table EMP(au niveau de la session 1).|
| t6 | ------ |```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem', 'Maaoui');```|La ligne ajoutée au niveau de la session 1 n'est pas visible(seulement Hichem et Mohamed) .|
| t7 | ------ |```UPDATE EMP SET SAL = 5000 WHERE ENAME ='Hichem';```|La requête ne s'exécute pas .|
| t8 | ```Commit;``` |------|Session 2: 1 row updated.|
| t9 | ------ |```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem', 'Maaoui');```|Après le commit, les 3 employés sont sélectionnés(Mohamed -->3800, Hichem --> 5000 et Maaoui). |
| t10| ------ |```COMMIT;```|------|
| t11| ```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem', 'Maaoui');```|------|La mise à jour du salaire de Hichem est donc visible dans la session 1.|
...
| t2| ------ |```SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;```|Le niveau d'isolation Serializable fournit l'isolation de transaction la plus stricte. Ce niveau simule l'exécution des transactions en série pour toutes les transactions validées; comme si les transactions avaient été exécutées les unes après les autres, en série, plutôt que simultanément.|
| t3| ------ |```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem');```|------|
| t4| ------ |```UPDATE EMP SET SAL = 3800 WHERE ENAME ='Mohamed';```|1 row updated au niveau de la session 2.|
| t5| ```Insert into EMP (EMPNO,ENAME,JOB,MGR,HIREDATE,COMM,DEPTNO) values ('9999','Maaoui','Magician',null,to_date('17/02/2021','DD/MM/RR'),null,'10');``` |------|1 row created au niveau de la session 1.|
| t6| ```COMMIT;```|------ |------|
| t7|```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem', 'Maaoui');```| ------ |Mohamed : 2000/Hichem :4000/Maaoui . (Les modifications au niveau de la session 2 ne sont pas exécutées).|
...
| t12| ------ | ```COMMIT;```|------|
| t13| ``` UPDATE EMP SET SAL = 5000 WHERE ENAME ='Maaoui'; ``` |------|1 row updated au niveau de la session 1.|
| t14| ------ |```SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;```|Transaction set(session 2).|
| t15| ------ |```UPDATE EMP SET SAL = 5200 WHERE ENAME ='Maaoui';```|La reqête est bloquée.|
| t16| ```COMMIT;``` |------|Erreur au niveau de la session 2 (isolation Serializable).|
| t17| ------ |```ROLLBACK;```|------|
| t18| ------ |```SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;```|Transaction set(session 2).|
| t19| ------ |```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem', 'Maaoui');```|Mohamed:2000/Hichem:4000/Maaoui:5000 .|
| t20| ``` UPDATE EMP SET SAL = 5200 WHERE ENAME ='Maaoui'; ``` |------|1 row updated (session 1).|
| t21| ```COMMIT;``` |------|------|
| t22| ------ | ```COMMIT;```|------|
| t23| ------ |```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem', 'Maaoui');```|Les transactions sont exécutées les unes après les autres, en série (Mohamed:2000/Hichem:4000/Maaoui:5200). |
